...
1) Be helpful and value-focused. Write comprehensive emails that demonstrate Cyfuture's expertise and value proposition.
2) Always keep a professional, engaging sales/marketing tone with persuasive language.
3) **For emails:** Include detailed information, company overview for new prospects, relevant links, and clear benefits.
4) When a customer asks for a demo, propose 2–3 time slots and offer a Meet/Teams link.
5) For pricing/proposals, query knowledge base first, then provide comprehensive, well-formatted details.
6) If the email shows high risk (legal, security, or compliance issues) or ambiguity you cannot resolve, escalate.
7) Summarize the source email before replying so a human can quickly review your reasoning.
8) When you send or draft any reply, write a crisp subject if you must set it explicitly.
9) Respect opt-outs. If the sender requests to unsubscribe or stop contact, mark as read and do not follow up.
10) Keep internal notes out of customer-facing content.
11) Fetch maximum 10 emails in a single tool call, do repetitive multiple tool calls to complete tasks if needed.
12) ALWAYS cite the knowledge base and web search when using information from it in customer communications.
...
Behaviour & Principles:
1) Be helpful and value-focused. Write comprehensive, informative emails that showcase Cyfuture's expertise.
2) Always maintain a professional, engaging tone with strong sales and marketing perspective.
3) **EMAIL WRITING - CRITICAL:**
   - **NEVER use placeholders** like [Your Name], [list services], [phone number], etc.
   - **ALWAYS use real information:** Your name is {user_name}, email is {user_email}
   - **For company info:** MUST query knowledge base FIRST to get real service details, pricing, features
   - **For introductions:** Query KB for Cyfuture services, then write compelling overview with specifics
   - Create detailed, well-structured emails with clear value propositions
   - Include relevant company information, product benefits, and technical details
   - Add appropriate links to services, documentation (e.g., https://www.cyfuture.com)
   - Use persuasive language that demonstrates Cyfuture's capabilities
   - Use web search for industry insights, competitor comparisons, market trends
   - ALWAYS cite sources when using KB or web search results
4) When a customer asks for a demo, propose 2–3 time slots and offer a Meet/Teams link.
5) For pricing or proposal inquiries: FIRST query knowledge base, use web search if helpful, then provide comprehensive formatted response.
6) If email shows high risk (legal, security, compliance) or unclear intent, escalate.
7) Summarize source email before replying for human review.
8) Write clear subjects for new emails or forwards.
9) Respect opt-outs: If sender requests unsubscribe/stop contact, mark read and don't follow up.
10) Keep internal notes out of customer-facing content.
11) Fetch maximum 10 emails per tool call, use multiple calls if needed.
12) Use dynamic_mail_fetch_tool for filtered searches.
13) Use reply_inline to maintain email threads.
14) Use good HTML formatting in emails (tables, headings, lists, etc.) to make them visually appealing

**AUTOPILOT & AUTOMATION:**
- Autopilot runs independently, processing new unread emails based on autopilot rules
- Tools: create_autopilot_rule, list_autopilot_rules, delete_autopilot_rule, create_action_plan, list_action_plans, delete_action_plan
- Action plans support natural language periodic task execution and stopping conditions (e.g., "Stop when user replies")
- Use these to automate follow-ups, summaries, and periodic tasks and other complex tasks
...
- **TOOL SELECTION** (Choose most efficient):
  
  **FOR SPECIFIC/SINGLE EMAIL** (e.g., "latest email", "email from John"):
  → dynamic_mail_fetch_tool (metadata only, lightweight)
  → THEN fetch_email if user wants to read/reply (single full email)
  
  **FOR MULTIPLE EMAILS WITH FULL CONTENT** (e.g., "check my 5 unread emails and reply"):
  → search_and_fetch_emails (gets 3-10 full emails at once)
  
  **GENERAL PRIORITY:**
  → dynamic_mail_fetch_tool (fastest, metadata only)
  → fetch_email (single email with full content)
  → few fetch_email calls > search_and_fetch_emails (multiple emails, use sparingly)
...
- **EMAIL WRITING**: Create detailed, informative emails with:
  * Clear value propositions and benefits
  * Relevant company/product information from KB
  * Links to services, case studies, documentation
  * Professional sales/marketing perspective
  * For new prospects: compelling company introduction
- Query KB for accurate pricing, features, technical details
- Use web search for industry insights, comparisons, market trends  
...
🤖 **ACTION PLANS (create_action_plan, list_action_plans, update_action_plan, delete_action_plan)**:
- **CRITICAL**: create_action_plan is for SCHEDULING tasks to run automatically later with stopping conditions.
...
- The system will automatically execute the plan at the scheduled time
- You can use both create_action_plan and create_autopilot_rule to complete user demand regarding unread mails and periodic tasks if needed.
- **EMAIL REFERENCE IN ACTION PLANS (IMPORTANT)**:
  * When creating action plans that reference specific emails (e.g., "send email to xyz and follow up every hour until reply received"):
    1. FIRST research well and then send asked mail to xyz
    2. THEN use dynamic_mail_fetch_tool to fetch email metadata (item_id, changekey, subject, sender)
    3. THEN include this metadata in the action plan prompt/task description
    4. Format: "Follow up on email [Subject: '...' | From: '...' | item_id: '...' | changekey: '...']"
    5. This allows the action plan executor to reference and track the specific email thread
  * Example workflow:
    User: "Send email to john@company.com and follow up every hour until he replies"
    ✅ Correct: dynamic_mail_fetch_tool(reciever="john@company.com") → get metadata → create_action_plan(task="Follow up on email [Subject: 'Demo Request' | To: 'john@company.com' | item_id: 'AAMk...' | changekey: 'CQA...'] every hour until reply received") → end_task(...)
    ❌ Wrong: create_action_plan(task="Follow up with john@company.com") (Missing email metadata!)
- Example workflow:
  User: "Schedule a daily email summary at 9 AM"
  ✅ Correct: create_action_plan(...) → end_task("Plan created, will run daily at 9 AM")
  ❌ Wrong: create_action_plan(...) → fetch emails → send summary (DON'T DO THIS!)
...
💡 EFFICIENCY REMINDER: Use the minimum tools needed to fulfill the user's actual request. Batch operations where possible.

🎯FOR YOUR KNOWLEDGE :
- Prospects means Potential Customers.
    ...
    def chatbot_react(state: State):
        ...
        for step in react_steps:
            if step.step_type == "thought":
                response_parts.append(f"💭 **Thought:** {step.content}")
            elif step.step_type == "action":
                response_parts.append(f"⚙️ **Action:** `{step.tool_name}`")
                if step.tool_input:
                ...
                response_parts.append(f"📊 **Observation:** {obs_preview}")
...
st.title("📧 AI Sales Representative Agent - ReAct Mode")
...
with st.sidebar:
    ...
    # )
    from autopilot import get_hands_free_mode, set_hands_free_mode

    current_hands_free = get_hands_free_mode()
    hands_free = st.toggle(
        "🚀 Hands-free Mode", 
        value=current_hands_free,
        key="hands_free_toggle",
        help="Auto-send routine emails without confirmation"
    )

    # Save state if changed
    if hands_free != current_hands_free:
        set_hands_free_mode(hands_free)
        st.rerun()
    
    st.markdown("---")
    st.markdown("### 🤖 Autopilot Service")
    ...
    if service_status == "running":
        ...
        st.info("⏸️ Service Stopped")
    elif service_status == "not_installed":
        st.warning("⚠️ Service Not Installed")
    else:
        ...
        if autopilot_service_enabled:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    st.error(f"❌ Failed to start service: {result.stderr}")
            except Exception as e:
                st.error(f"❌ Error starting service: {e}")
        else:
            # Stop service
            try:
                result = subprocess.run(
    ...
    if autopilot_service_enabled or service_status == "running":
        ...
        with col1:
            if st.button("💾 Save", use_container_width=True):
                ...
                st.success("✅ Saved - restart service for changes")
        ...
        try:
            ...
            if last_run:
                ...
                try:
                    ...
                    st.caption(f"⏱️ Last run: {dt.strftime('%Y-%m-%d %H:%M:%S')}")
                except:
                    st.caption(f"⏱️ Last run: {last_run}")
        except Exception:
            pass
        
        # Manual run button
        if st.button("▶️ Run Now (Manual)", use_container_width=True):
            with st.spinner("Running autopilot..."):
                try:
                    logs = autopilot_once()
                    st.session_state.autopilot_logs.extend(logs)
                    st.success(f"✅ Processed {len(logs)} items")
                except Exception as e:
    ...
    if service_status == "not_installed":
        st.markdown("---")
        st.info("💡 Run `install_autopilot_service.bat` to install the service")
...
with tab1:
                ...
                with st.chat_message("assistant"):
                    ...
                    for line in lines:
                        if line.startswith("💭 Thought:"):
                            thought_counter += 1
                            st.markdown(line)
                        elif line.startswith("⚙️ Action:"):
                            action_counter += 1
                            action_name = line.replace("⚙️ Action:", "").strip()
                            with st.expander(f"⚙️ **Action {action_counter}:** {action_name}", expanded=True):
                                st.caption("Tool call details available in live view")
                        elif line.startswith("📊 Observation:"):
                            obs_content = line.replace("📊 Observation:", "").strip()
                            # Show preview in expander
                            preview = obs_content[:100] if len(obs_content) > 100 else obs_content
                            with st.expander(f"📊 **Observation {action_counter}:** {preview}...", expanded=False):
                                st.text_area(
                                    "Output",
                                    obs_content,
                                    height=200,
                            ...
                            final_content = "\n\n".join(lines[final_idx:]).replace("✅ Final Answer:", "").strip()
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            thinking_placeholder.info("🤔 Starting ReAct loop...")
            ...
            try:
                for step in react_agent.run_streaming(
                    ...
                    with main_container:
                        ...
                        if step.step_type == "thought":
                            thought_counter += 1
                            st.markdown(f"💭 **Thought {thought_counter}:** {step.content}")
                        
                        # ACTIONS - Collapsible/Expandable
                        elif step.step_type == "action":
                            action_counter += 1
                            with st.expander(f"⚙️ **Action {action_counter}:** `{step.tool_name}`", expanded=False):
                        ...
                        # OBSERVATIONS - Collapsible/Expandable with FULL content
                            ...
                            with st.expander(
                                f"📊 **Observation {action_counter}:** {len(step.content)} chars | {preview}...", 
                                expanded=False
                            ):
                        ...
                        # ERROR - Show prominently with warning
                            ...
                            if "Context Length Exceeded" in step.content:
                                if st.button("🧹 Clear Chat Now", key=f"clear_error_{time.time()}"):
                                    st.session_state.messages = []
                                    st.rerun()
            
            except Exception as e:
                # Catch any errors that weren't caught inside run_streaming
                error_str = str(e)
                is_context_error = (
                    "maximum context length" in error_str.lower() 
                    or "context length is" in error_str.lower()
                    or ("tokens" in error_str.lower() and "input" in error_str.lower())
                )
                
                thinking_placeholder.empty()
                
                if is_context_error:
                    st.error("⚠️ **Context Length Exceeded!**")
                    st.warning(
                        f"The conversation has become too long. The model can only handle **31,768 tokens**, "
                        f"but this conversation has exceeded that limit.\n\n"
                        f"**Error Details:** {error_str}\n\n"
                        f"**Solution:** Click the button below to clear the chat history and start fresh."
                    )
                    if st.button("🧹 Clear Chat History", key=f"clear_chat_error_{time.time()}"):
                        st.session_state.messages = []
                        st.success("✅ Chat cleared! You can now start a new conversation.")
                        st.rerun()
                else:
                    st.error(f"❌ **An error occurred:** {error_str}")
                
                # Don't continue processing - skip the rest
                pass
            
            else:
                # Only process if no exception occurred
    ...
    # Instructions
    with st.expander("ℹ️ How to Use ReAct Mode"):
        st.markdown("""
        **ReAct Pattern** = **Reasoning** + **Acting**
        
        The agent will show you its thinking process:
        1. 💭 **Thought:** What the agent is thinking
        2. ⚙️ **Action:** Which tool it's using
        3. 📊 **Observation:** The result from the tool
        4. ... (repeats as needed)
        5. ✅ **Final Answer:** The conclusion
...
with tab2:
    ...
    with col1:
        ...
        if available_collections:
            selected_collection = st.selectbox(
                "🎯 Active Knowledge Base Collection",
            ...
            st.warning("⚠️ No collections found in Qdrant. Upload documents to create one.")
    ...
    with col2:
        ...
        if st.button("✅ Set Active", use_container_width=True):
            if selected_collection and selected_collection != "None":
                ...
                st.success(f"✅ Active collection: **{selected_collection}**")
                st.rerun()
            else:
                set_active_collection(None)
                st.info("ℹ️ No collection active")
                st.rerun()
                ...
                with st.spinner("Processing documents and building vector store..."):
                    try:
                        ...
                        if doc_splits:
                            ...
                            st.success(f"✅ Knowledge base **{collection_name}** built with {len(doc_splits)} chunks!")
                            st.balloons()
    ...
    with col_manage:
        st.markdown("### 🔍 Test Knowledge Base")
        
        active = get_active_collection()
        if active:
            st.info(f"📚 Querying: **{active}**")
                    ...
                    with st.spinner("Searching knowledge base..."):
                        try:
                            ...
                            if "error" in result_data:
                                ...
                                st.success(f"✅ Found {len(result_data.get('hits', []))} results")
            ...
            st.warning("⚠️ No active collection. Upload documents or select an existing collection.")
    
    st.markdown("---")
    
    # Collection Management
    st.markdown("### 🗂️ Manage Collections")
    ...
    if available_collections:
        for col_name in available_collections:
            ...
            with col2:
                if st.button("📊 Info", key=f"info_{col_name}"):
                    try:
            ...
            with col3:
                if st.button("🗑️ Delete", key=f"del_{col_name}"):
                    try:
...
# ==========================================
with tab3:
    st.markdown("## ⚙️ Autopilot Rules Configuration")
    ...
    st.caption("💡 Priority: 1=Critical, 2=Medium, 3=Low. Used only when rules contradict each other.")
    ...
    # Save button
    if st.button("💾 Save All Rule Changes", type="primary", use_container_width=True):
        set_autopilot_rules(current_rules)
    ...
    with st.form("new_rule_form"):
        ...
        if submit:
            if not new_name.strip() or not new_prompt.strip():
                st.warning("⚠️ Please provide both name and instruction")
            else:
                new_rule = {
                ...
                set_autopilot_rules(current_rules)
                st.success(f"✅ Added rule: '{new_rule['name']}'")
                st.rerun()
...
# ==========================================
with tab4:
    st.markdown("## 📊 Autopilot Activity Logs")
    ...
    if summaries:
        for s in summaries[:10]:
            with st.expander(f"⏰ {s.get('time', '')} — {s.get('subject', '')}"):
...
# ==========================================
with tab5:
    st.markdown("## 🎯 Action Plans Management")
    ...
    st.markdown("### 📋 Active Action Plans")
    ...
    for idx, plan in enumerate(current_plans):
        with st.container():
            ...
            with col2:
                ...
                st.caption(plan.get("task", "—"))
            
            with col3:
                # Use formatter for better display
                freq_display = format_frequency_display(plan)
                st.caption(f"📅 {freq_display}")
            
            with col4:
                if st.button("🗑️", key=f"plan_remove_{plan['id']}"):
                    to_delete_plan_index = idx
        
        st.markdown("---")
    
    if not current_plans:
        st.info("📝 No action plans yet. Create one below!")
    
    if to_delete_plan_index is not None:
        removed = current_plans.pop(to_delete_plan_index)
        set_action_plans(current_plans)
        st.success(f"✅ Removed plan '{removed.get('name')}'")
        st.rerun()
    
    # Save button
    if current_plans and st.button("💾 Save Action Plan Changes", type="primary", use_container_width=True):
        set_action_plans(current_plans)
        st.success("✅ Action plans saved!")
    
    st.markdown("---")
    
    # Add new plan
    st.markdown("### ➕ Create New Action Plan")
    st.info("💡 **Tip:** Use the chatbot to create action plans with advanced scheduling (hourly, daily, custom intervals in minutes/hours/days)")
    ...
    with st.form("new_plan_form"):
        ...
        st.markdown("#### ⏹️ Stopping Condition (Optional)")
        stopping_cond = st.text_area(
            "Stopping Condition",
            placeholder='e.g., "Stop after 5 executions" or "Stop when customer replies" or "Stop after December 31, 2024"',
            help="Natural language description of when this plan should stop automatically",
            height=80
        )
        ...
        auto_delete = st.checkbox(
            ...
            value=False,
            help="If checked, plan will be deleted when condition is met. Otherwise, it will be disabled."
        )
        
        enabled_new_plan = st.checkbox("Enable immediately", value=True)
        
        submit_plan = st.form_submit_button("➕ Add Action Plan", use_container_width=True)
        
        if submit_plan:
            if not new_plan_name.strip() or not new_plan_task.strip():
                st.warning("⚠️ Please provide both name and task")
            else:
                new_plan = {
                    ...
                    "stopping_condition": stopping_cond.strip() if stopping_cond.strip() else None,
                    "auto_delete_on_stop": bool(auto_delete),
                    "last_executed": None,
                    "created_at": datetime.now(timezone.utc).isoformat()
                }
                current_plans.append(new_plan)
                set_action_plans(current_plans)
                st.success(f"✅ Added action plan: '{new_plan['name']}'")
                st.rerun()
    ...
    # EXECUTION HISTORY
    # ==========================================
    st.markdown("## 📊 Action Plan Execution History")
                ...
                if entry.get("steps"):
                    ...
                    for idx, step in enumerate(entry.get("steps", []), 1):
                        ...
                        if step_type == "thought":
                            st.caption(f"{idx}. 💭 {content}")
                        elif step_type == "action":
                            st.caption(f"{idx}. ⚙️ {content}")
...
# ==========================================
with tab6:
    st.markdown("## 🔌 Exchange Web Services Connection")
    ...
    try:
        ...
        st.error("⚠️ ews_config.py not found - credential persistence disabled")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("###  📧 EWS Configuration")
        ...
        with col_test:
            if st.button("🧪 Test & Save Connection", type="primary", use_container_width=True):
                if not input_email or not input_pwd:
                    ...
                    st.error("❌ Cannot save - ews_config module not available")
                else:
                            ...
                            with st.spinner("Saving credentials..."):
                                ...
                                if save_success:
                                    ...
                                    if reload_credentials():
                                        st.success("✅ Credentials saved and applied!")
                                        st.info("ℹ️ No restart needed - credentials active now")
                                    else:
                                        st.warning("⚠️ Credentials saved but failed to reload. Please restart app.")
                                else:
                                    st.error("❌ Connection succeeded but failed to save credentials")
                        else:
                            st.error(message)
                            st.warning("⚠️ Credentials NOT saved (connection test failed)")
        
        with col_save:
            if st.button("💾 Save Without Testing", use_container_width=True):
                if not input_email:
                    st.warning("⚠️ Email address is required")
                elif not ews_config_available:
                    st.error("❌ Cannot save - ews_config module not available")
                else:
                    ...
                    if success:
                        ...
                        if reload_credentials():
                            ...
                            st.warning("⚠️ Credentials not tested - verify connection before use")
                        else:
                            st.warning("⚠️ Settings saved but failed to reload. Please restart app.")
                    else:
                        st.error("❌ Failed to save settings")
        
        # Quick fetch test
        st.markdown("---")
        st.markdown("### 📬 Quick Email Fetch Test")
        
        if st.button("📥 Fetch 5 Unread Emails", use_container_width=True):
            try:
    ...
    with col2:
        st.markdown("### ℹ️ Connection Info")
        
        # Show current settings
        current_creds = {
            "Email": current_email or "Not set",
            "Host": current_host or "Auto-detect",
            "Password": "●●●●●●" if os.getenv("EWS_PASSWORD") else "Not set"
        }
        ...
        st.success(
            ...
            "4. Use '📥 Fetch Emails' to test retrieval"
        )
...
if st.session_state.get(AUTO_KEY):
    ...
    if not last or (now - last).total_seconds() > (period_min * 60):
        ...
        if not st.session_state.get(AUTO_KEY):
            ...
            with st.spinner("🤖 Running autopilot sweep..."):
                try:
                    ...
                    if not st.session_state.get(AUTO_KEY):
                        st.info("⏸️ Autopilot stopped by user after email processing")
                    else:
                        ...
                        for plan in enabled_plans:
                            ...
                            if not st.session_state.get(AUTO_KEY):
                                st.info("⏸️ Autopilot stopped by user during action plan execution")
                                break
