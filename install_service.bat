@echo off
REM Install Action Plan Service as Windows Service using NSSM
REM
REM Prerequisites:
...
echo ========================================
echo Action Plan Service Installer
echo ========================================
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo Please uninstall it first using uninstall_service.bat
    echo.
    pause
    exit /b 1
)
...
REM Install service
nssm install %SERVICE_NAME% "%PYTHON_EXE%" "%SERVICE_DIR%action_plan_service.py"
...
REM Set description
nssm set %SERVICE_NAME% Description "Action Plan Background Service - Executes scheduled action plans automatically"

REM Set display name
nssm set %SERVICE_NAME% DisplayName "Action Plan Service"

REM Configure logging
nssm set %SERVICE_NAME% AppStdout "%SERVICE_DIR%action_plan_service_stdout.log"
nssm set %SERVICE_NAME% AppStderr "%SERVICE_DIR%action_plan_service_stderr.log"
...
echo   - %SERVICE_DIR%action_plan_service.log (main log)
echo   - %SERVICE_DIR%action_plan_service_stdout.log (stdout)
echo   - %SERVICE_DIR%action_plan_service_stderr.log (stderr)
echo.
pause
